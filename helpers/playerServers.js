...
import { getItem, setItem, localeHHMMSS, createUUID } from '/common/common.js'
...
async function buyServer(purchasedServers, serverMap, didChange) {
...
async function upgradeServer(ns, purchasedServers, didChange, serverMap) {
  ...
  return {purchasedServers, didChange};
}
...
export async function main(ns) {
  ...
  while (!endProgram) {
    ...
    if (action == settings.actions.BUY) {
      didChange = await buyServer(purchasedServers, serverMap, didChange);
    } else {
      const __ret = await upgradeServer(ns, purchasedServers, didChange, serverMap);
      purchasedServers = __ret.purchasedServers;
      didChange = __ret.didChange;
    }
